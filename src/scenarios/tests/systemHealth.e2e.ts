...
export const systemHealthE2ETest = {
  ...
  fn: async (runtime: IAgentRuntime) => {
    ...
    try {
      ...
      const healthAction = runtime.actions.find(
        action => action.name === "SYSTEM_HEALTH_CHECK"
      );
      ...
      const healthProvider = runtime.providers.find(
        p => p.name === "SYSTEM_HEALTH_CONTEXT"
      );
