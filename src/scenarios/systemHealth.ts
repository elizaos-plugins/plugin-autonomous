...
export const systemHealthProvider: Provider = {
  ...
  get: async (runtime: IAgentRuntime, message: Memory, state: State) => {
    ...
    const activeTasks = healthTasks.filter((task) => !task.tags?.includes('completed'));
...
export const systemHealthCheckAction: Action = {
  ...
  validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
    ...
    return (
      ...
      (text.includes('health') || text.includes('check') || text.includes('status'))
    );
  },
  ...
  handler: async (
    ...
    try {
      ...
      const taskId = await runtime.createTask({
        ...
        description:
          'Check disk space, memory usage, CPU load, and create maintenance tasks if needed',
      ...
      const thought =
        "I'll perform a comprehensive system health check, monitoring disk space, memory, and CPU usage. I'll create maintenance tasks for any issues found.";
